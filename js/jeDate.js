...
(function(win) {
	...
	*/ 
	...
	//显示隐藏层
	jeDt.dateShdeCell = function(type) {
    ...
    var config = {
        ...
        zIndex:999,   //弹出层的层级高度
        choose:function(val) {}  //选择时间之后的回调函数
	...
	var jeDate = function(options) {       
        return new InitDate(options || {});  //核心方法
    };
	InitDate.prototype = {
        init:function() {
        ...
        setHtml:function(opts, self) {
            var that = this, weekHtml = "", date = new Date(), nowDateVal = jeDt.nowDate(null, opts.format);
             var initVal = opts.isinitVal ?
                 jeDt.isValHtml(self) ? jeDt.val(self) :jeDt.text(self) : 
				 ((jeDt.val(self) || jeDt.text(self)) == "") ? nowDateVal : jeDt.isValHtml(self) ? jeDt.val(self) :jeDt.text(self);
			...
			'<div class="jedateym"><i class="prev triangle yearprev"></i><span class="jedateyy" data-ym="24"><em class="jedateyear"></em><em class="pndrop"></em></span><i class="next triangle yearnext"></i></div>' + 
			'<div class="jedateym"><i class="prev triangle monthprev"></i><span class="jedatemm" data-ym="12"><em class="jedatemonth"></em><em class="pndrop"></em></span><i class="next triangle monthnext"></i></div>' + 
			'</div>'+
			'<div class="jedatetopym" style="display: none;">'+
			    '<ul class="ymdropul"></ul><p><span class="jedateymchle">&#8592;</span><span class="jedateymchri">&#8594;</span><span class="jedateymchok">关闭</span></p>'+
			'</div>' + 
			'<ol class="jedaol"></ol>' + 
			'<ul class="jedaul"></ul>'+
			'<div class="jedatebot">'+
			...
			if(opts.isTime){
				...
				QD(Cell+" .jedatebot .jedatehms")[0].style.display = "none";
				QD(Cell+" .jedatebot .jedatebtn")[0].style.width = "100%";
			}
            jeDt.dateShdeCell(false);
		...
		getDateStr:function(y, m, d) {
			...
			//是否显示节日
			var isfestival = function(day, n){ return opts.festival ? jeDt.festival(day,n) : n; }
		...
		events:function(that, opts, self, arrTime) {			
			...
			jeDt.each(QD(Cell+" .jedatebot .jedatehms em"), function(i, cls) {				
				jeDt.on(cls, "click", function() {
					var hmsStr = "",acton, hmscell = QD(Cell+" .jedateprophms")[0],  hmslen = jeDt.attr(this, "data-hms"), hmsstxt = ['小时', '分钟', '秒数'],
					...
					jeDt.each(QD(Cell+" .jedateprophms p"), function(i, p) {	
						jeDt.on(p, "click", function() {
							jeDt.html(cls, jeDt.digit(jeDt.text(this)));
							removeEmpty();
						});
					}); 
			...
			jeDt.on(QD(Cell+" .jedatebot .jedateclear")[0], "click", function() {
				jeDt.val(self,'');
				jeDt.html(QD(Cell)[0], '');
                jeDt.dateShdeCell(true);  
			});
			//今天
			jeDt.on(QD(Cell+" .jedatebot .jedatetoday")[0], "click", function() {
                ...
                jeDt.html(QD(Cell)[0], '');  jeDt.dateShdeCell(true);  
			...
			jeDt.on(QD(Cell+" .jedatebot .jedateok")[0], "click", function(ev) {
				...
				var getokDate = (jeDt.val(self) != '' || jeDt.text(self) != '') ? jeDt.parse([ okTime[0], okTime[1], okTime[2] ], [ okTimeArr[0], okTimeArr[1], okTimeArr[2] ], opts.format) : "";
				that.getDateStr(okTime[0], okTime[1], okTime[2]);
				jeDt.each(QD(Cell+" .jedaul li"), function(i, cls) { 
					if(jeDt.attr(cls, "class") != 'action'){return;}else{
						jeDt.isValHtml(self) ? jeDt.val(self, getokDate) :jeDt.text(self, getokDate);
					}
				})
				jeDt.html(QD(Cell)[0], '');   jeDt.dateShdeCell(true);   
			});
		...
		YearAndMonth : function(that, opts, self, arrTime){
			var jetopym = QD(Cell+" .jedatetopym")[0], jedateyy = QD(Cell+" .jedateyy")[0], jedatemm = QD(Cell+" .jedatemm")[0],
			jedateyear = QD(Cell+" .jedateyy .jedateyear")[0], jedatemonth = QD(Cell+" .jedatemm .jedatemonth")[0],
			mchri = QD(Cell+" .jedateymchri")[0], mchle = QD(Cell+" .jedateymchle")[0];	
			...
			function setYearMonth(YY,ymlen){
				...
				if(ymlen == 12){	
					...
					ymStr = eachYears(YY);  
					jeDt.each([ mchri, mchle ], function(c,cls){cls.style.display = "block"});
				}
			...
			jeDt.on(jedateyy, "click", function() {
				var YMlen = parseInt(jeDt.attr(this, "data-ym")), yearAttr = parseInt(jeDt.attr(jedateyear,"data-year"));
		...
		clickLiDays : function(that, opts, self){
            jeDt.each(QD(Cell+" .jedaul li"), function(i, cls) {
                jeDt.on(cls, "click", function(ev) {
					...
					var liTimeArr = [];
					jeDt.each(QD(Cell+" .jedatehms em"), function(l, tval) {
						liTimeArr.push(jeDt.text(tval));
					})
					var aty = parseInt(jeDt.attr(this, "data-y")) | 0, atm = parseInt(jeDt.attr(this, "data-m")) | 0, atd = parseInt(jeDt.attr(this, "data-d")) | 0;
					var getParDate = jeDt.parse([ aty, atm, atd ], [ liTimeArr[0], liTimeArr[1], liTimeArr[2] ], opts.format);
